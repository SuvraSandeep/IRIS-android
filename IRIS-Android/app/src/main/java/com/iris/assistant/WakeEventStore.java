...
final class WakeEventStore {
    static final class Event {
        ...
        final long at=SystemClock.elapsedRealtime();final float[] embedding;final boolean accepted;float[][] sound;
    ...
    static synchronized void addSound(String reason,String revision,float[] vector,boolean accepted,float[][] sound){
        add(reason,revision,vector,accepted);Event event=events.peekFirst();
        if(SoundPattern.valid(sound)){event.sound=new float[sound.length][];for(int i=0;i<sound.length;i++)event.sound[i]=sound[i].clone();}
    }
    ...
    private static void erase(Event e){if(e.embedding!=null)Arrays.fill(e.embedding,0);if(e.sound!=null)for(float[] row:e.sound)Arrays.fill(row,0);}
