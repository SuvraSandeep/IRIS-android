...
Missing identity/model, invalid vectors and exceptions must reject wake. Quiet speech never bypasses owner checks. As authorized in the September 2026 rebuild, authenticated sentence enrollment collects usable speaker evidence without requiring exact ASR text. Held-out wake verification still requires the complete typed phrase and owner match; save only after validation and device authentication. Phrase preview alone cannot save identity. Command practice and generic imports cannot replace owner identity. WakeChangeApproval may only be entered from explicit authenticated MainActivity actions.
